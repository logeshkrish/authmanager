...
func (backend *JWTAuthenticationBackend) SetValueInRedis(tokenString string, token *jwt.Token) error {
	...
	log.Errorln("redisConn val inside jwt  ", redisConn)
	// if claims, ok := token.Claims.(*jwt.StandardClaims); ok && token.Valid {
	// 	log.Debug(" %v", claims.ExpiresAt)

	claims := token.Claims.(jwt.MapClaims)
	// fmt.Println("Claims---------")
	log.Errorln("expires in ", time.Unix(int64(claims["exp"].(float64)), 0))
...
func (backend *JWTAuthenticationBackend) ReadValueFromRedis(token string) string {
	...
	if err != nil {
		if logs.IsDebugEnabled {
			log.Errorln(" Error While Reading Redis Value: ", err.Error())
	...
	// log.Errorln("found in redis[jwt]")
...
func getPrivateKey() *rsa.PrivateKey {
	...
	if err != nil {
		log.Errorln("failed on decoding a private key", err)
	}
